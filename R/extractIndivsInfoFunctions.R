...
#'
#' @param sf_EndByStage - \pkg{sf} dataframe returned by \code{\link{extractIndivInfoEndByStage}}
#'
#'@return \pkg{sf} dataframe with columns:
#'\itemize{
#'  \item{startTime}
...
#'\code{\link{extractIndivInfoEndByStage}}.
...
extractIndivInfo_End<-function(sf_EndByStage){
  ...
  df_idC  = df_MaxAge %>% subset(!(origID==endID));
  origIDs = sort(unique(df_idC$origID));
  df_idNC = df_MaxAge %>% subset(!(origID %in% origIDs));
...
#'@export
#'
extractIndivInfo_StartEnd<-function(sf_start=NULL,
                                    sf_end=NULL,
                                    lst_indivs=NULL,
                                    startLHS=NULL,
                                    checkCalcs=FALSE){
...
#sf_se = extractIndivInfo_StartEnd(sf_start,sf_end,checkCalcs=TRUE);
